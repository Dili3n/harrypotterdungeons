...
# MODIFIE 26 / 01 / 2021
# Cela fait maintenant 6 mois que j'ai commencé le python
# Dans une école de licence Maths/Info. 
# Je vous propose mon tout premier projet 100% personnel. Il est fini... 
...
filepfc = 'assets/musique/pfc_music.mp3'
...
def sauvegarde(experience,niveau,salle,horcrux,ballais,energie,gallions,force,chance,pets_lst,ind):
    ...
    informations.
    """
    save = [experience,niveau,salle,horcrux,ballais,energie,gallions,force,chance,pets_lst,ind] 
    fichier = open("save.txt", "wb")
    pickle.dump(save,fichier)
    fichier.close()
...
def stats():
    ...
    """
    Fonction pour afficher les statistiques graphiques du jeu.
    """
...
def affiche_accueil(file):
    ...
    """
    Fonction qui affiche graphiquement l'accueil
    """
...
def clic_accueil(event):
    ...
    """
    Fonction qui prend en compte les clic de la souris pour les traduire
    si celle-ci est sur un bouton
    """
...
def affiche_menu(experience,niveau,force,chance,pets_lst):
    ...
    if pets_lst == 0:
        ...
        image(640,360,'assets/bgdebut1.png', ancrage = "center")
    elif pets_lst == 1:
        image(640,360,'assets/bgdebut2.png', ancrage = "center")
    elif pets_lst == 2:
        image(640,360,'assets/bgdebut3.png', ancrage = "center")
    ...
    if niveau < 5:
        image(995,540,'assets/button6.png', ancrage = "center")
    if niveau >= 5:
        image(995,500,'assets/button7.png', ancrage = "center")
    if niveau < 3:
        image(295,540,'assets/button8.png', ancrage = "center")
    if niveau >= 3:
        image(295,500,'assets/button5.png', ancrage = "center")


def affiche_xp_bar(experience,niveau):

    """
    Fonction qui permet de gerer la barre d'xp du menu.
    """
...
def clic_menu(event,ballais,horcrux,gallions,niveau,ind):

    """
    Gère les clic effectué sur le menu.
    """
    if 569 <= abscisse(event) <= 724 and 250 <= ordonnee(event) <= 479:
        if horcrux < 7:
            return False, True, False, False, False, ballais, False, False, False, ind,gallions
        else: return False, False, False, False, False, ballais, True, False, False, ind,gallions
    elif 217 <= abscisse(event) <= 372 and 180 <= ordonnee(event) <= 400:
        return False, False, True, False, False, ballais, False, False, False, ind,gallions
    elif 917 <= abscisse(event) <= 1072 and 180 <= ordonnee(event) <= 400:
        return False, False, False, True, False, ballais, False, False, False, ind,gallions
    elif 10 <= abscisse(event) <= 35 and 670 <= ordonnee(event) <= 720 and ballais >= 1:
        return False, False, False, False, True, ballais-1, False, False, False, ind,gallions 
    elif 245 <= abscisse(event) <= 345 and 440 <= ordonnee(event) <= 560 and gallions >= 50 and niveau >= 3:
        pygame.mixer.music.stop
        pygame.mixer.music.load(filepfc)
        pygame.mixer.music.play(-1)
        return False,False, False, False, False, ballais, False, True, False, ind,gallions
    elif 945 <= abscisse(event) <= 1045 and 440 <= ordonnee(event) <= 560 and niveau >= 5:
         return False, False, False, False, False, ballais, False, False, True, ind,gallions    
    elif 690 <= abscisse(event) <= 710 and 540 <= ordonnee(event) <= 560 and ind[0] < 2:
        ind[0] += 1
        return True, False, False, False, False, ballais, False, False, False, ind,gallions+100
    else: return True, False, False, False, False, ballais, False, False, False, ind,gallions
...
def chance_entraineur():
    ...
    """
    Gère la chance pour savoir contre qui va tomber l'aventurier.
    """
...
def can_hit(energie, endurance_monstre, combat_random,salle,force,chance):
    ...
    if chance_tape == 1 or chance_tape == 3:
        degat = (randint(5,20)*((force//100)+1))+pets_lst*5
        endurance_monstre -= degat
        fuite = aff_degat_joueur(degat, combat_random, energie, endurance_monstre)
        return energie, endurance_monstre,fuite
    else:
        degat = randint(1,5)*((salle//2)+1)
        energie -= degat
        fuite = aff_degat_monstre(degat, combat_random, energie, endurance_monstre)
        return energie, endurance_monstre,fuite 
...
def aff_degat_joueur(degat, x, energie, endurance_monstre):
    ...
    stats() 
    texte(1220,680,str("Fuite"),taille=16,couleur="saddle brown")
    texte(720,380,(degat)) 
    ...
    texte(750,570,(energie))
    mise_a_jour()
    abss, ordd = attend_clic_gauche()
    return clic_jeu(abss,ordd) 
...
def clic_jeu(abss,ordd):

    temp = randint(1,2)
    if temp == 1 and 1220 <= abss <= 1280 and 680 <= ordd <= 700:
        pygame.mixer.music.stop()
        pygame.mixer.music.load(file)
        pygame.mixer.music.play(-1)
        return True
...
def level_up(experience, niveau, force, chance,energie):
    ...
    if experience >= 100:
        ...
        force += randint(1,15)
        chance += randint(1,15)
    return experience, niveau, force, chance, energie

def affiche_pfc():

    image(640,360,'assets/bg_pfc.png', ancrage = "center")
    image(1230,670,'assets/croix.png', ancrage = "center")
    mise_a_jour()

def clic_pfc(event, gallions):

    if 1180 <= abscisse(event) <= 1280 and 620 <= ordonnee(event) <= 720 or gallions < 50:
        pygame.mixer.music.stop()
        pygame.mixer.music.load(file)
        pygame.mixer.music.play(-1)
        return False, True, 0
    elif 290 <= abscisse(event) <= 395 and 240 <= ordonnee(event) <= 320 and gallions >= 50:
        return True, False, 1
    elif 890 <= abscisse(event) <= 985 and 255 <= ordonnee(event) <= 325 and gallions >= 50:
        return True, False, 3
    elif 590 <= abscisse(event) <= 685 and 245 <= ordonnee(event) <= 355 and gallions >= 50:
        return True, False, 2
    else: return True, False, 0

def game_pfc(main_joueur,main_ron):

    if main_joueur == main_ron:
        return "egalite"
    elif main_joueur == 1 and main_ron == 3:
        return "joueur"
    elif main_joueur == 3 and main_ron == 2:
        return "joueur"
    elif main_joueur == 2 and main_ron == 1:
        return "joueur"
    elif main_ron == 1 and main_joueur == 3:
        return "ron"
    elif main_ron == 3 and main_joueur == 2:
        return "ron"
    elif main_ron == 2 and main_joueur == 1:
        return "ron"

def gagne_pfc(x,gallions,experience,niveau):

    if x == "ron":
        gallions -= 50
        if niveau < 10:
            experience += 10
        image(640,360,'assets/pfc_perdu.png', ancrage = "center")
    elif x == "joueur":
        gallions += 50
        if niveau < 10:
            experience += 20
        image(640,360,'assets/pfc_gagne.png', ancrage = "center")
    mise_a_jour()
    attend_clic_gauche
    return gallions, experience


def ron_pfc():

    main_ron = randint(1,3)
    if main_ron == 1:
        image(640,360,'assets/pfc_pierre.png', ancrage = "center")
    elif main_ron == 2:
        image(640,360,'assets/pfc_feuille.png', ancrage = "center")
    elif main_ron == 3:
        image(640,360,'assets/pfc_ciseaux.png', ancrage = "center")
...
def affiche_pets():
    ...
    image(640,360,'assets/bg_pets.png', ancrage = "center")
    image(1230,670,'assets/croix.png', ancrage = "center")
    mise_a_jour()

def clic_pets(event,gallions,pets_lst):

    if 1180 <= abscisse(event) <= 1280 and 620 <= ordonnee(event) <= 720:
        return False, True, pets_lst, gallions
    elif 310 <= abscisse(event) <= 410 and 405 <= ordonnee(event) <= 550 and gallions >= 850:
        gallions -= 850
        return True, False, 3, gallions
    elif 560 <= abscisse(event) <= 660 and 405 <= ordonnee(event) <= 550 and gallions >= 250:
        gallions -= 250
        return True, False, 1, gallions
    elif 810 <= abscisse(event) <= 910 and 405 <= ordonnee(event) <= 550 and gallions >= 550:
        gallions -= 550
        return True, False, 2, gallions
...
while main:
    ...
    while menu:
        ...
        affiche_menu(experience,niveau,force,chance,pets_lst)
        mise_a_jour()

        experience, niveau, force, chance, energie = level_up(experience, niveau, force, chance, energie)
        sauvegarde(experience,niveau,salle,horcrux,ballais,energie,gallions,force,chance,pets_lst,ind)
        ...
        if type == 'ClicGauche':
            menu,jeu,boutique,entrainement,coffre,ballais,boss,pfc,pets,indice,gallions = clic_menu(event, ballais, horcrux, gallions, niveau,ind)
    ...
    while jeu:
        ...
        while endurance_monstre > 0 and energie > 0 and fuite == False:
            energie, endurance_monstre,fuite = can_hit(energie, endurance_monstre, combat_random, salle,force,chance)

        if endurance_monstre <= 0 and fuite == False:
            victoire(energie)
            salle += 1
            print(salle)
            jeu, coffre = False, True

        elif energie <= 0 and fuite == False:
            jeu = defaite(energie, combat_random)
            experience, niveau, salle, horcrux, ballais, energie, gallions,force,chance,pets_lst = 0,1,0,0,0,100,200,0,0,0
            fin_game()
    ...
    while boss:
        ...
        if endurance_monstre <= 0:
            ...
            jeu = defaite(energie, combat_random)
            experience, niveau, salle, horcrux, ballais, energie, gallions,force,chance,pets_lst = 0,0,0,0,0,100,200,0,0,0
            fin_game()
            jeu, accueil = False,True
    ...
    while pfc:
       ...
       if type == 'Quitte':
            main, accueil, menu, entrainement, boutique, jeu, boss, pfc = leave()

       if type == 'ClicGauche':
           pfc, menu, main_joueur = clic_pfc(event, gallions)
           if main_joueur != 0:
               main_ron = ron_pfc()
               sleep(1)
               gagnant_pfc = game_pfc(main_joueur,main_ron)
               gallions, experience = gagne_pfc(gagnant_pfc,gallions,experience, niveau)
    ...
    while pets:
       ...
       if type == 'Quitte':
            main, accueil, menu, entrainement, boutique, jeu, boss, pets = leave()

       if type == 'ClicGauche':
            pets, menu, pets_lst, gallions = clic_pets(event,gallions,pets_lst)
